 ...
 * (WyattAu Kit Compatibility Matrix). Versions verified against the
 * crates.io API on 2026-09-26. Update alongside KITS.md.
 */
...
export const KITS: Kit[] = [
  ...
  },
  {
    ...
    consumers: ["ferro", "aether-core", "fetch-kit"],
  },
  {
    name: "throttle-kit",
    repo: "ratelimit",
  ...
  },
  {
    ...
    version: "0.1.0",
    layer: "L2",
    purpose: "Lock-free Prometheus-exposition metrics — counters, gauges, histograms with cardinality guards, VictoriaMetrics-compatible output",
    consumers: ["telemetry-init"],
  },
  {
    name: "config-kit",
    version: "0.1.1",
    layer: "L2",
    purpose: "Layered typed configuration — file → env → overrides, unknown-field denial, secret redaction, hot reload",
    consumers: [],
  },
  {
    ...
    version: "0.1.0",
    layer: "L2",
    purpose: "Idempotency keys — scoped key derivation, TTL claims with response replay",
    consumers: [],
    features: "memory / redis / sqlite stores",
  },
  {
    name: "outbox-kit",
    version: "0.1.1",
    layer: "L2",
    purpose: "Transactional outbox — durable event envelopes, at-least-once dispatch with breaker-aware backoff, replay after restart",
    consumers: [],
  },
  {
    name: "percentile-kit",
    version: "0.1.0",
    layer: "L2",
    purpose: "Latency percentile tracking — P50/P99/P99.9 from criterion output, committed budgets, CI regression enforcement",
    consumers: [],
  },
  {
    name: "chaos-kit",
    version: "0.1.1",
    layer: "L2",
    purpose: "Deterministic fault injection — latency, errors, partitions, resource starvation with seeded scheduling",
    consumers: [],
  },
  {
    name: "telemetry-init",
    version: "0.1.0",
    layer: "L2",
    purpose: "One-call observability bootstrap — tracing + metrics + OTLP wired with estate defaults",
    consumers: [],
  },
  {
    name: "worker-kit",
    version: "0.1.0",
    layer: "L2",
    purpose: "Periodic job scheduling — jittered intervals, graceful drain, breaker-aware failure budgets, optional leader election",
    consumers: [],
  },
  ...
  },
  {
    ...
    version: "0.1.4",
    layer: "L3",
    purpose: "Resilient outbound HTTP client — retry, circuit breaker, connection pooling, typed JSON (supersedes resilient-fetch)",
    consumers: [],
  },
  {
    name: "ws-barbican",
