...
1. **Cadence.** Every kit carries `.github/workflows/mutation.yml` — weekly
   schedule (Mon 03:00 UTC) + `workflow_dispatch`. The job is
...
3. **Triage discipline.** Every survived mutant is classified as exactly one
   of:
   - **MISSING TEST** → write a killing test, confirm with a scoped
     `cargo mutants --file <f>` re-run, land under
     `test: kill <n> surviving mutants (cargo-mutants)`.
